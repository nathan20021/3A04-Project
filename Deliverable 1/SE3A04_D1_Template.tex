...
\begin{enumerate}[a)]
	\item The product being developed is an Android application which empowers the ability to book carpools via a user-friendly interface. The application will securely store customer personal information such as carpool request histories, and personal data inputted by the user. The product is not self-contained since its functionality depends on Google Maps for mapping integration. The product scope mainly covers a match making functionality to match potential carpoolers together. The feature will be implemented under a ccentrelized dispatcher which communicate with the mobile app and user’s database. Fare determination will be covered under the scope of the app, however payment processing is external to the system.
...
\begin{enumerate}[a)]
	...
	\item Riders might be sensitive with prices and therefore opt out for carpools instead of regular taxi services.
...
\begin{enumerate}[a)]
	...
	\begin{itemize}
		\item Users have an android mobile device. If users uses IOS or other operating system, the app would not be available.
...
\item[$S_{4.1}$] System sends customer's request to carpool offerer
        ...
        \item[Global] Scenario for BE3
            \begin{enumerate}
                ...
                \item[$E_1$] Customer inputs destination
                \item[$S_{2.1}$] System computes estimated payment split based on taxi company's fares
                \item[$S_{2.2}$] System asks cutomers to input search criteria
		    ...
		    \item[$S_{5.1}$] System sends customer's request to carpool offerer
                ...
                \end{enumerate}
                \item[$E_5$] Customers can track location of designated cab
            \end{enumerate}
    \end{enumerate}
    ...
    \begin{enumerate}[{VP5}.1]
        ...
        \item[Global] Scenario for BE5
            \begin{enumerate}
                \item[$S_1$] System displays fare to each customer
                \item[$E_1$] Customer pays taxi driver
                \begin{enumerate}
                    \item[$E_{1.1}$] Drivers obtain payment from customers [out of scope for system]
                    \item[$E_{1.2}$] Driver input that payment is complete
                \end{enumerate}
                \item[$S_2$] Once payment approved, system prompts customer to rate the other customers they shared a ride with, with an option to leave a review
